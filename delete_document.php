...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $query = "DELETE FROM documents WHERE id=$announcement_id";
    $result = mysqli_query($db, $query);

    if ($result) {
        echo "Το εγγραφο διαγράφηκε επιτυχώς!";
    } else {
        echo "Κάτι πήγε στραβά κατά τη διαγραφή του εγγράφου " . mysqli_error($db);
    }
} else {
